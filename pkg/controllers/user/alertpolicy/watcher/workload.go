...
func StartWorkloadWatcher(ctx context.Context, cluster *config.UserContext, manager *manager.AlertManager) {
	...
	projectAlerts := cluster.Management.Management.ProjectAlertPolicies("")
	d := &WorkloadWatcher{
		projectAlertPolicies:     projectAlerts,
		projectAlertPolicyLister: projectAlerts.Controller().Lister(),
...
func (w *WorkloadWatcher) watchRule() error {
	...
	for _, alert := range pAlerts {
		...
		for _, targetWorkload := range alert.Spec.TargetWorkloads {
			if targetWorkload.WorkloadID != "" {

				wl, err := w.workloadController.GetByWorkloadID(targetWorkload.WorkloadID)
				if err != nil {
					...
					logrus.Warnf("Fail to get workload for %s, %v", targetWorkload.WorkloadID, err)
					continue
				}

				w.checkWorkloadCondition(alert.Name, alert.Namespace, wl, &targetWorkload)

			} else if targetWorkload.Selector != nil {
				...
				for _, n := range namespaces {
					...
					wls, err := w.workloadController.GetWorkloadsMatchingSelector(namespace.Name, targetWorkload.Selector)
					if err != nil {
...
func (w *WorkloadWatcher) checkWorkloadCondition(name, namespace string, wl *workload.Workload, alert *v3.TargetWorkload) {
	...
	percentage := alert.AvailablePercentage
