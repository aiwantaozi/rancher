...
func (m *AlertManager) GetAlertManagerEndpoint() (string, error) {
	svc, err := m.svcLister.Get(monitorutil.SystemMonitoringNamespaceName, "alertmanager-svc")
	if err != nil {
...
func (m *AlertManager) GetState(groupID string, apiAlerts []*APIAlert) string {

	for _, a := range apiAlerts {
		if string(a.Labels["group_id"]) == groupID {
...
func (m *AlertManager) AddSilenceRule(groupID string) error {
...
func (m *AlertManager) RemoveSilenceRule(groupID string) error {
	url, err := m.GetAlertManagerEndpoint()
	if err != nil {
		return err
	}
	...
	q := req.URL.Query()
	q.Add("filter", fmt.Sprintf("{%s}", "group_id="+groupID))
