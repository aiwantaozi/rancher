...
func (w *NodeWatcher) watchRule() error {
	...
	for _, alert := range clusterAlerts {
		if alert.Status.State == "inactive" || alert.Spec.NodeRule == nil {
			continue
		}
		...
		if alert.Spec.NodeRule.NodeName != "" {
			...
			if machine == nil {
				deleteRuleIDs = append(deleteRuleIDs, alert.Name)
				continue
			}
			w.checkNodeCondition(groupID, alert, machine)
...
func (w *NodeWatcher) checkNodeCondition(groupID string, alert *v3.ClusterAlertRule, machine *v3.Node) {
	switch alert.Spec.NodeRule.Condition {
	case "notready":
		w.checkNodeReady(groupID, alert, machine)
	case "mem":
		w.checkNodeMemUsage(groupID, alert, machine)
	case "cpu":
		w.checkNodeCPUUsage(groupID, alert, machine)
	}
}

func (w *NodeWatcher) checkNodeMemUsage(groupID string, alert *v3.ClusterAlertRule, machine *v3.Node) {
	if v3.NodeConditionProvisioned.IsTrue(machine) {
		...
		if used.Value()*100.0/total.Value() > int64(alert.Spec.NodeRule.MemThreshold) {
			...
			// data["alert_name"] = alert.DisplayName
...
func (w *NodeWatcher) checkNodeCPUUsage(groupID string, alert *v3.ClusterAlertRule, machine *v3.Node) {
	if v3.NodeConditionProvisioned.IsTrue(machine) {
...
func (w *NodeWatcher) checkNodeReady(groupID string, alert *v3.ClusterAlertRule, machine *v3.Node) {
