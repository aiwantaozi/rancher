...
func (w *kafkaTestWrap) TestReachable(dial dialer.Dialer, includeSendTestLog bool) error {
	...
	if w.ZookeeperEndpoint != "" {
		...
		conn, err := newConn(dial, "tcp", url.Host, tlsConfig, true)
		if err != nil {
			return err
		}
		conn.Close()
...
func (w *kafkaTestWrap) kafkaConn(dial dialer.Dialer, config *tls.Config, smartHost string) (*kafka.Conn, error) {
	...
	conn, err := newConn(dial, "tcp", smartHost, config, false)
	if err != nil {
		return nil, err
	}
	...
	LeaderConn, err := newConn(dial, "tcp", leaderSmartHost, config, false)
	if err != nil {
		return nil, err
	}
