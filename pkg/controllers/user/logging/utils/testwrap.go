...
func NewLoggingTargetTestWrap(es *v3.ElasticsearchConfig, sp *v3.SplunkConfig, sl *v3.SyslogConfig, kf *v3.KafkaConfig, ff *v3.FluentForwarderConfig) loggingTargetTestWrap {
	if es != nil {
		return &elasticsearchTestWrap{es}
	} else if sp != nil {
		return &splunkTestWrap{sp}
	} else if sp != nil {
		return &syslogTestWrap{sl}
	} else if kf != nil {
		return &kafkaTestWrap{kf}
	} else if ff != nil {
		return &fluentForwarderTestWrap{ff}
	}
...
func (w *fluentForwarderTestWrap) sendData2Server(dial dialer.Dialer, shareKey, username, password, endpoint, hostname, certificate string) error {
	...
	conn, err := newConn(dial, "tcp", endpoint, certificate, "", "", "", "", hostname, false) //todo EnableTLS
	if err != nil {
		return err
	}
...
func testReachableHTTP(dial dialer.Dialer, req *http.Request, rootCA, clientCert, clientKey, clientKeyPass, sslVersion string, sslVerify bool) error {
	...
	if _, err = client.Do(req); err != nil {
		return fmt.Errorf("request to logging target %s failed, error: %v", req.URL, err)
	}
	return nil
}
...
func sendData2Kafka(topic string, partition int, dialer dialer.Dialer, protocal, smartHost, rootCA, clientCert, clientKey, clientKeyPass, sslVersion string, sslVerify bool) error {
	...
	if _, err = kafkaConn.WriteMessages(
		kafka.Message{Value: []byte("Rancher logging target setting validated!")},
	); err != nil {
