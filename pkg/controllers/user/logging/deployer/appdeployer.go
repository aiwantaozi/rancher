...
func (d *AppDeployer) cleanup(appName, appTargetNamespace, projectID string) error {
	...
	errgrp.Go(func() error {
		return d.AppsGetter.Apps(projectName).Delete(appName, &metav1.DeleteOptions{})
	})
	...
	return &projectv3.App{
		...
		Spec: projectv3.AppSpec{
			Answers: map[string]string{
				...
				`fluentd-tester.secrets.cluster\.conf`: clusterConfig,
				`fluentd-tester.secrets.project\.conf`: projectConfig,
			},
