...
<filter {{$store.ProjectName}}.** project-custom.{{$store.ProjectName}}.** {{ if $store.IsSystemProject }}rke.**{{end-}} >
  @type record_transformer
  <record>
...
<match  {{$store.ProjectName}}.** project-custom.{{$store.ProjectName}}.** {{ if $store.IsSystemProject }}rke.**{{end-}}> 
