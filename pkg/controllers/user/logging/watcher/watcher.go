...
func (e *endpointWatcher) checkTarget() error {
	...
	wl := utils.NewLoggingTargetTestWrap(obj.Spec.ElasticsearchConfig, obj.Spec.SplunkConfig, obj.Spec.SyslogConfig, obj.Spec.KafkaConfig, obj.Spec.FluentForwarderConfig, obj.Spec.CustomTargetConfig)
	if wl == nil {
		err = nil
	} else {
	...
	for _, v := range pls {
		wp := utils.NewLoggingTargetTestWrap(v.Spec.ElasticsearchConfig, v.Spec.SplunkConfig, v.Spec.SyslogConfig, v.Spec.KafkaConfig, v.Spec.FluentForwarderConfig, obj.Spec.CustomTargetConfig)
		if wp == nil {
			err = nil
		} else {
