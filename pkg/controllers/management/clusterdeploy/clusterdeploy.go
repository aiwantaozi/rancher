...
func Register(ctx context.Context, management *config.ManagementContext, clusterManager *clustermanager.Manager) {
	c := &clusterDeploy{
		...
		clusters:             management.Management.Clusters(metav1.NamespaceAll),
		templateVersions:     management.Management.TemplateVersions(metav1.NamespaceAll),
		nodeLister:           management.Management.Nodes(metav1.NamespaceAll).Controller().Lister(),
		clusterManager:       clusterManager,
		projectsGetter:       management.Management.(mgmtv3.ProjectsGetter),
...
func (cd *clusterDeploy) sync(key string, cluster *mgmtv3.Cluster) (runtime.Object, error) {
	var (
		err, updateErr error
	)
...
func (cd *clusterDeploy) doSync(cluster *mgmtv3.Cluster) error {
	if !mgmtv3.ClusterConditionProvisioned.IsTrue(cluster) {
		return nil
	}
	...
	_, err = mgmtv3.ClusterConditionSystemAccountCreated.DoUntilTrue(cluster, func() (runtime.Object, error) {
...
func (cd *clusterDeploy) deployAgent(cluster *mgmtv3.Cluster) error {
	...
	_, err = mgmtv3.ClusterConditionAgentDeployed.Do(cluster, func() (runtime.Object, error) {
		...
		mgmtv3.ClusterConditionAgentDeployed.Message(cluster, string(output))
		return cluster, nil
	})
	if err != nil {
		return err
...
func (cd *clusterDeploy) setNetworkPolicyAnn(cluster *mgmtv3.Cluster) error {
...
func (cd *clusterDeploy) getKubeConfig(cluster *mgmtv3.Cluster) (*clientcmdapi.Config, error) {
	systemUser, err := cd.systemAccountManager.GetSystemUser(cluster)
	if err != nil {
		return nil, err
	}

	token, err := cd.userManager.EnsureToken("agent-"+systemUser.Name, "token for agent deployment", systemUser.Name)
	if err != nil {
		return nil, err
	}
...
func (cd *clusterDeploy) getYAML(cluster *mgmtv3.Cluster, agentImage string) ([]byte, error) {
	...
	if url == "" {
		return nil, errors.New(fmt.Sprintf("waiting for server-url setting to be set"))
	}
...
func (cd *clusterDeploy) deployPrometheusOperator(cluster *mgmtv3.Cluster) (string, error) {
...
func ensureMonitoringProjectName(agentNamespacesClient corev1.NamespaceInterface, cattleProjectsClient mgmtv3.ProjectInterface, clusterName, appTargetNamespace string) (string, error) {
	// detect Namespace
...
func grantSystemMonitoringRBAC(agentServiceAccountGetter corev1.ServiceAccountsGetter, agentRBACClient rbacv1.Interface, appName, appTargetNamespace string) (string, error) {
...
func deploySystemMonitoring(cattleAppsGetter projectv3.AppsGetter, cattleTemplateVersionsClient mgmtv3.TemplateVersionInterface, clusterCreatorID, appProjectName string, appName, appTargetNamespace, appServiceAccountName string) error {
