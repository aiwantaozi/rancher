...
func (n *RKENodeConfigServer) nonWorkerConfig(ctx context.Context, cluster *v3.Cluster, node *v3.Node) (*rkeworker.NodeConfig, error) {
	...
	for _, tempNode := range plan.Nodes {
		if tempNode.Address == node.Status.NodeConfig.Address {
			nc.Processes = augmentProcesses(token, tempNode.Processes, false, infos[tempNode.Address])
			return nc, nil
		}
	}
...
func (n *RKENodeConfigServer) nodeConfig(ctx context.Context, cluster *v3.Cluster, node *v3.Node) (*rkeworker.NodeConfig, error) {
	...
	if hostDockerInfo.OSType == "windows" {
	...
	for _, tempNode := range plan.Nodes {
		if tempNode.Address == hostAddress {
			nc.Processes = augmentProcesses(token, tempNode.Processes, true, infos[tempNode.Address])
...
func augmentProcesses(token string, processes map[string]v3.Process, worker bool, hostDockerInfo types.Info) map[string]v3.Process {
	var shared []string

	if isB2DOS := strings.Contains(hostDockerInfo.OperatingSystem, rkehosts.B2DOS); isB2DOS {
	...
	if isWindowsOS := strings.Contains(hostDockerInfo.OperatingSystem, rkehosts.WindowsOS); !isWindowsOS { // doesn't consider with windows
