...
func (s *Handler) handler(apiContext *types.APIContext) error {
	...
	endpoint, err := getPrometheusEndpoint(userContext, params.clusterName)
	if err != nil {
		return err
	}

	ctx := context.Background()
	resourceType := strings.TrimSuffix(params.resourceType, "stats")
	manager, err := NewManager(ctx, dial, resourceType, endpoint)
	if err != nil {
		return err
	}

	metric, err := manager.GetMetric(params)
	if err != nil {
		return err
	}
...
func getPrometheusEndpoint(userContext *config.UserContext, clusterName string) (string, error) {
	svcs, err := userContext.Core.Services("").Controller().Lister().List("cattle-prometheus", prometheusLabels.AsSelector())
...
func parseParams(query url.Values, reqURL string) (*params, error) {
	...
	case workloadstats:
		...
		workloadName = id
		_, namespace, podName, err = parseWorkloadName(id)
		if err != nil {
			return nil, err
		}
