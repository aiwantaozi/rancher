...
type Handler struct {
	ClusterAlertPolicies v3.ClusterAlertPolicyInterface
...
func (h *Handler) ClusterActionHandler(actionName string, action *types.Action, request *types.APIContext) error {
	...
	alert, err := h.ClusterAlertPolicies.GetNamespaced(ns, id, metav1.GetOptions{})
	if err != nil {
	...
	alert, err = h.ClusterAlertPolicies.Update(alert)
	if err != nil {
...
func (h *Handler) ProjectActionHandler(actionName string, action *types.Action, request *types.APIContext) error {
	...
	alert, err := h.ProjectAlertPolicies.GetNamespaced(ns, id, metav1.GetOptions{})
	if err != nil {
		logrus.Errorf("Error while getting alert for %s :%v", request.ID, err)
		return err
	}
	...
	alert, err = h.ProjectAlertPolicies.Update(alert)
	if err != nil {
		logrus.Errorf("Error while updating alert:%v", err)
		return err
	}
